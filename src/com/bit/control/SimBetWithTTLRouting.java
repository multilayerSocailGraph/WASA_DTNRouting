...
public class SimBetWithTTLRouting
{
	ArrayList<Couple> coupleList = new ArrayList<Couple>();
	ArrayList<Contact> contactList = new ArrayList<Contact>();

	public SimBetWithTTLRouting(int test_startTime, int test_endTime)//初始化messageList和contactList
	{
		...
		for(int i = 0; i < coupleList.size(); i++)//开始使用SimBet算法发送每一个消息
		{
			...
			for(int j = 0; j < contactList.size(); j++)
			{
				...
				if(preNode!=null && node!=null)
				{
					...
					{//还没转发到目的节点
						double ttl = (double)preNode.msgQueue.getFirst().TTL;
						...
						double simBetUtil_preNode = 0.5 * simUtil_preNode + 0.5 * betUtil_preNode;
						double simBetUtil_node = 0.5 * simUtil_node + 0.5 * betUtil_node;
						
						if(simBetUtil_preNode < simBetUtil_node)
						{
							preNode.msgQueue.getFirst().flag++;
							if(preNode.msgQueue.getFirst().flag > 1)
							{
								double alpha = preNode.msgQueue.getFirst().flag;
								double temp = 1 + (ttl/(double)Message.MAXTTL)*(1/alpha)*0.9;
								simBetUtil_preNode = temp*simBetUtil_preNode;
							}
							else
							{
								double temp = 1 + (ttl/(double)Message.MAXTTL)*0.9;
								simBetUtil_preNode = temp*simBetUtil_preNode;
							}
						}
						...
						if(simBetUtil_preNode < simBetUtil_node)
						{
							preNode.msgQueue.getFirst().flag = 0;
							preNode.msgQueue.getFirst().TTL--;
	...
	{	
		...
		double deliverRatio = (double)Main.successArray[i]/(double)Main.coupleArray[i];
		double averageDelay = (double)Main.delayArray[i]/(double)Main.successArray[i];
		double averageHops = (double)Main.hopArray[i]/(double)Main.successArray[i];
		double averageForwards = (double)Main.forwardArray[i]/(double)Main.coupleArray[i];
		System.out.println("\nSimBetWithTTLRouting---第"+(i+1)+"次测试总数:"+Main.coupleArray[i]);
		...
		if(i==9)
		{
			RoutingPerformanceFrame frame = new RoutingPerformanceFrame(Main.load, "SimBetWithTTL Routing", Main.successArray[i],
