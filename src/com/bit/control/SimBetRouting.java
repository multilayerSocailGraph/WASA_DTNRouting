...
public class SimBetRouting 
{
	ArrayList<Couple> coupleList = new ArrayList<Couple>();
	ArrayList<Contact> contactList = new ArrayList<Contact>();

	public SimBetRouting(int test_startTime, int test_endTime)//初始化messageList和contactList
	{
	...
	public void execute()
	{
		for(int i = 0; i < coupleList.size(); i++)//开始使用SimBet算法发送每一个消息
		{
			...
			{
				if(contactList.get(j).firstNode.name.equals("38") && contactList.get(j).secondNode.name.equals("32")
						&& contactList.get(j).startTime==249022 && couple.srcNode.name.equals("35")
						&& couple.desNode.name.equals("79"))
				{
					System.out.println("jfsa");
				}
	...
	{	
		...
		System.out.println("\nSimBetRouting---第"+(i+1)+"次测试总数:"+Main.coupleArray[i]);
		...
		if(i==9)
		{
			RoutingPerformanceFrame frame = new RoutingPerformanceFrame(Main.load, "SimBet Routing", Main.successArray[i],
