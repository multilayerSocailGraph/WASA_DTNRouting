...
public class FreshRouting 
{
	ArrayList<Couple> coupleList = new ArrayList<Couple>();
	ArrayList<Contact> contactList = new ArrayList<Contact>();

	public FreshRouting(int test_startTime, int test_endTime)//初始化messageList和contactList
	{
			...
			for(int j = 0; j < contactList.size(); j++)
			{
				...
				if(preNode!=null && node!=null)
				{
					...
					{//还没转发到目的节点
						...
						if(Main.contactFreshMatrix[preNode.indexInValidNodes][desNode.indexInValidNodes] < Main.contactFreshMatrix[node.indexInValidNodes][desNode.indexInValidNodes])
						{
							preNode.msgQueue.getFirst().TTL--;
	...
	{	
		...
		System.out.println("\nFreshRouting---第"+(i+1)+"次测试总数:"+Main.coupleArray[i]);
		...
		if(i==9)
		{
			RoutingPerformanceFrame frame = new RoutingPerformanceFrame(Main.load, "Fresh Routing", Main.successArray[i],
